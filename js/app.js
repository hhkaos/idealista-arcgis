...
angular.module('idealista-arcgis', ['esri.map', 'ngSanitize'])
    .controller('MapController', function ($scope, esriRegistry) {
      ...
      /*
       * Creamos un objeto esriObjs para mantener los objetos de Esri fuera del scope del callback de esriRegistry.
       * Anteriormente se utilizaba $scope para esto, pero esos objetos no tienen ninguna interacción con la vista,
       * son para uso interno, por lo que no hay necesidad de meterlos en el scope.
       */
      var esriObjs = {};
      ...
      esriRegistry.get('map').then(function(map) {
        require([
          ...
          // Asignamos los objetos de esri al objeto esriObjs para su posterior uso
          esriObjs = {
            "Map":Map,
          ...
          var orangeRed = new esriObjs.Color([238, 69, 0, 0.5]);
          $GEO.marker = new esriObjs.SimpleMarkerSymbol("solid", 10, null, orangeRed);
          var renderer = new esriObjs.SimpleRenderer($GEO.marker);
        ...
        map.on('click', function(e) {
          ...
          var LongLat = esriObjs.WebMercatorUtils.xyToLngLat(point.x, point.y);
          ...
          var loc = new esriObjs.Point(
              $scope.evt.click.lng,
              $scope.evt.click.lat
          );

          var symbol = new esriObjs.PictureMarkerSymbol("img/pin.png", 16, 24);
          capaGrafica.add(new esriObjs.Graphic(loc, symbol, poi));
        });
      });
      ...
      var paintResults = function(result){
        ...
        for(i=0; i<len; i++){
          ...
          el[i]
          var loc = new esriObjs.Point(el[i].longitude, el[i].latitude);
          capaGrafica.add(new esriObjs.Graphic(loc, $GEO.marker, el[i]));
        }
        $scope.$apply();
        /*
        $scope.$apply(function(){
      ...
      var endpointRequest = function(poiId) {
        ...
        var deferred = esriObjs.esriRequest({
      ...
      $scope.search = function(){
        ...
        setTimeout(function() {
          esriObjs.allDojo(promises).then(function(results)
          {
